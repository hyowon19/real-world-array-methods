// ==========================================================================
// DecodeMTL instructors solutions - Workshop 4 - Real World Array Methods
// ==========================================================================
...
function willItRainToday(data) {
  var isGoingToRain = data.findIndex(function(datum) {
...
console.log(willItRainToday(weatherData.hourly.data));
...
console.log(willItRain8Hours(weatherData.hourly.data));


// Just give me the temperature
function justTemperature(data) {
  return data.map(function(datum) {
    return datum.temperature;
  });
}
console.log(justTemperature(weatherData.hourly.data));


// When is it going to rain?
function whenWillItRain(data) {
  return (
    data.filter(function(datum) {
      return datum.summary==="Rain" || datum.summary==="Light Rain";
    })
    .map(function(datum) {
      return new Date(datum.time);
    })
  );
}
console.log(whenWillItRain(weatherData.hourly.data));


// Is it going to be sunny all week?
function sunnyAllWeek(data) {
  return (
    data.map(function(datum) {
      if (datum.summary.includes('sun')) {
        return true;
      }
      return false;
    })
...
console.log(sunnyAllWeek(weatherData.daily.data));


// How hot will it be today?
function howHotToday(data) {
  return data.reduce(function(hottest, datum) {
    if (datum.temperature > hottest) {
      return datum.temperature;
    }
    return hottest;
  }, -Infinity);
}
console.log(howHotToday(weatherData.hourly.data));


// Using the appropriate array method, start with the daily data and find out
// how many times each icon -- representing the weather condition for that
// day -- appears.
function weekStats(data) {
  return data.reduce(function(stats, datum) {
    if (stats[datum.icon]) {
      stats[datum.icon]++;
    }
    else {
...
console.log(weekStats(weatherData.daily.data));
...
// As an additional hint, the first method is filter and the last one is
// forEach. Up to you to figure out the two middle ones.
...
function longChain(data) {
  function onlyUnique(value, index, self) {
    return self.indexOf(value) === index;
  }
  return (
    data.filter(function(person) {
      return person.email.slice(person.email.length-4) === ".biz";
    })
    .map(function(person) {
...
console.log(longChain(peopleInfo));
